...
All notable changes to SacredCows will be documented in this file.
...
of the Milk bucket and produce a normal milk bucket with no teleport abilities.
This has been fixed.
...
- Severe bug that crashed single-player client instances on startup. Moved the
  call to load `cowPositions` inside the `SERVER_STARTED` registration
